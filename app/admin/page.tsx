...
export default async function AdminPage({ searchParams }: Props) {
  ...
  const filters = {
    q: sp.q?.trim() ?? '',
    country: sp.country ?? '',
    withResearchers: (['yes', 'no'].includes(sp.with ?? '')
      ? (sp.with as 'yes' | 'no')
      : 'all') as 'yes' | 'no' | 'all',
    sort: (sp.sort === 'count' ? 'count' : 'name') as 'name' | 'count',
    dir: (sp.dir === 'desc' ? 'desc' : 'asc') as 'asc' | 'desc',
  };
  ...
  // Aplicar filtros (solo super admin tiene filtros; institution admin
  // tiene 1-2 instituciones y no los necesita)
  ...
  // Lista de países que aparecen en las instituciones (para el filtro)
    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      <header className="mb-6">
        ...
        <p className="mt-1 text-sm text-[var(--muted)]">
          {user.isSuperAdmin
            ? `Estás conectado como super administrador. ${totalCount} instituciones, ${totalResearchers} investigadores en total.`
            : `Estás conectado como administrador. Gestionas ${allInstitutions.length} institución(es).`}
        </p>
      </header>
      ...
      {user.isSuperAdmin && (
        <>
          <AdminInstitutionFilters
            countries={countriesInUse}
            initial={filters}
          />
          {(filters.q ||
            filters.country ||
